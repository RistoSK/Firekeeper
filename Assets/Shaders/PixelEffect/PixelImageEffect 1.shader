Shader "Custom/PixelImageEffectWithLUT"
{
    Properties
    {
        ...
        Pass
        {
           ...
           // #pragma fragment frag
            ...
            fixed4 frag (v2f_img i) : SV_Target
            {
				...
				float xOffset = halfColX + c.r * threshold / COLORS;
				float yOffset = halfColY + c.g * threshold;
				float cell = floor(c.b * maxColor);

				float2 lutPos = float2(cell / COLORS + xOffset, yOffset);
				float4 gradedCol = tex2D(_LUT, lutPos);
				float4 final = lerp(c, 1-gradedCol, _LUTAmount);
				return final;
            }
            ENDCG
        }
