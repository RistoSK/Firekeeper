    ...
    public class MeshAnimationCreator : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            scroll = GUILayout.BeginScrollView(scroll);
            {
                ...
                if (prefab != null)
                {
                    if (GUILayout.Button(string.Format("Clear Preferences for {0}", prefab.name)))
                    {
                ...
                if (prefab != null)
                {
                    ...
                    {
                        rootMotionMode = MeshAnimation.RootMotionMode.None;
                    }
                    ...
                    if (extrasExposed != null)
                    {
                        ...
                        if (anyMissing)
                        {
                            string message = "<b>Missing Exposed transforms in the Rig Import Settings";
                            for (int i = 0; i < missing.Count; i++)
                            {
                                message += string.Format("\n - {0}", missing[i]);
                            }
                            message += "</b>";
                            EditorGUILayout.LabelField(message, GUI.skin.FindStyle("CN EntryWarn"));
                            GUILayout.Space(15);
                        }
                    }
        ...
        private void DrawText(string text, Color color)
        {
            ...
            GUILayout.TextArea(text);
            GUI.contentColor = orignalContentColor;
        }
        #endregion
        ...
        private void CreateSnapshots()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    // store triangle data (for nintendo switch fix specifically)
                    ma.meshTriangleData = new MeshTriangleData[ma.baseMesh.subMeshCount];
                    for (int i = 0; i < ma.baseMesh.subMeshCount; i++)
                    {
                        MeshTriangleData triangleData = new MeshTriangleData() { submesh = i };
                        triangleData.triangles = ma.baseMesh.GetTriangles(i);
                        ma.meshTriangleData[i] = triangleData;
                    }
                    ma.FPS = fps;
        ...
        /// Combines the prefab meshes into a single mesh, only combining if needed
