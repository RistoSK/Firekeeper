...
public class PrepareToBuildWallState : BuildingBaseState
{
    ...
    public PrepareToBuildWallState(Dictionary<HexDirection, GameObject> tempWalls) : base()
    {
    ...
    public override Type Tick()
    {
        if (Input.GetMouseButton(0) && !EventSystem.current.IsPointerOverGameObject())
        {
            ...
            if (Physics.Raycast(inputRay, out var hit, 1000f, BuildModeSettings.CellLayerMask))
            {
                ...
                if (_isDrag)
                {
                    ...
                    Cell.ShowTempWall(_tempWalls, _dragDirection);
        ...
        if (Input.GetMouseButtonUp(0) && _isDragConfirmed)
        {
            ...
            Cell.AddWall(_currentCellDirection, BuildModeSettings.Wall);
            ChangeCellColor(Cell, Color.white);
